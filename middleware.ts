...
export default async function middleware(request: NextRequest) {
  ...
  const { value: token } = cookies.get("token") ?? { value: null }; //*value keyini desctruct edip alıyoruz ad olarak da token olarak alıyoruz yani value as token;
...
export const config = {
  matcher: ["/sign-in", "/sign-up", "/premium"],
